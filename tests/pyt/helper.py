...
def example_graph():
    ...
    graph.add_edge(start, cities,
                   rule=Transformation(1, ["city(a)", "city(b)", "city(c)", "city(d)"]))
    roads = fromPrev({"road(a,b,10)", "road(b,c,20)", "road(c,d,25)", "road(d,a,40)", "road(b,d,30)", "road(d,c,25)"},
                     cities)
    graph.add_edge(cities, roads, rule=Transformation(2,
                                                      ["road(a,b,10)", "road(b,c,20)", "road(c,d,25)", "road(d,a,40)",
                                                       "road(b,d,30)", "road(d,c,25)"]))
    gen = Transformation(3, ["{ travel(X,Y) } :- road(X,Y,_)."])
    ...
    visited = Transformation(4, ["visited(Y) :- travel(X,Y), start(X).", "svisited(Y) :- travel(X,Y), visited(X)."])
    ...
    constraint = Transformation(5,
                                [":- city(X), not visited(X).", ":- city(X), 2 { travel(X,Y) }.",
                                 ":- city(X), 2 { travel(Y,X) }."])
    ...
    minimize = Transformation(6, ["#minimize { D,X,Y : travel(X,Y), road(X,Y,D) }."])
